...
// r2z_256_32x32: 32x32 block tiles for better parallelism at size 256
//
// TUNING: 32x32 blocks give 64 blocks at 256x256 (vs 16 with 64x64)
//         TM=4, TN=4 gives 64 threads = (32/4)*(32/4) = 8*8
//         2 warps per block, 16 accumulators/thread
//
// Key optimizations:
//   - Double-buffer SMEM: As[2][BK*BM], Bs[2][BK*BN] = 8 KB total
//   - B tiles loaded via __pipeline_memcpy_async
//   - A tiles use float4 global loads + scatter-transpose into SMEM

namespace r2z_256_32x32_config {
    constexpr int BM = 32;   // 32x32 blocks
    constexpr int BN = 32;
    constexpr int BK = 16;
    constexpr int WM = 32;
    constexpr int WN = 32;
    constexpr int WNITER = 1;
    constexpr int TM = 4;    // 4x4 thread tiles
    constexpr int TN = 4;
    constexpr int NUM_THREADS = 64;  // (32/4)*(32/4) = 8*8 = 64
}

constexpr const char* const VARIANT_ID   = "r2z_256_32x32";
constexpr const char* const VARIANT_DESC = "32x32_BK16_64t_db_async";
...
void launch_gemm_fp32_r2z_256_32x32(
    const float* d_A,
    const float* d_B,
    float*       d_C,
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    using namespace r2z_256_32x32_config;
...
const char* get_variant_id_fp32_r2z_256_32x32()   { return VARIANT_ID; }
const char* get_variant_desc_fp32_r2z_256_32x32() { return VARIANT_DESC; }
