...
// r2z_256_opt3: Option 3 - 64x64 tiles with larger warp tiles
//
// TUNING: WM=64, WN=64 with WNITER=2 for more warp-level parallelism
//         TM=4, TN=8 gives 128 threads = (64/4)*(64/8) = 16*8
//         Different thread tile shape (4x8 vs 8x4)
//
// Key optimizations:
//   - Double-buffer SMEM: As[2][BK*BM], Bs[2][BK*BN] = 32 KB total
//   - B tiles loaded via __pipeline_memcpy_async
//   - A tiles use float4 global loads + scatter-transpose into SMEM

namespace r2z_256_opt3_config {
    constexpr int BM = 64;
    constexpr int BN = 64;
    constexpr int BK = 32;
    constexpr int WM = 64;   // Larger warp tiles
    constexpr int WN = 64;
    constexpr int WNITER = 2;
    constexpr int TM = 4;    // Smaller TM
    constexpr int TN = 8;    // Larger TN
    constexpr int NUM_THREADS = 128;  // (64/4)*(64/8) = 16*8 = 128
}

constexpr const char* const VARIANT_ID   = "r2z_256_opt3";
constexpr const char* const VARIANT_DESC = "64x64_WM64_128t_db_async_opt3";
...
void launch_gemm_fp32_r2z_256_opt3(
    const float* d_A,
    const float* d_B,
    float*       d_C,
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    using namespace r2z_256_opt3_config;
...
const char* get_variant_id_fp32_r2z_256_opt3()   { return VARIANT_ID; }
const char* get_variant_desc_fp32_r2z_256_opt3() { return VARIANT_DESC; }
