...
constexpr const char* VARIANT_DESC = "128x64_tiles_larger";

template <int BM, int BN, int BK, int TM, int TN, int THREADS>
__global__ void gemm_128x64_kernel(
    ...
    int* bank_conflict_count)
{
    ...
    for (int bk = 0; bk < K; bk += BK) {
        ...
        const int row_per_warp = 32 / (BN / TN);
        for (int kk = 0; kk < BK; kk++) {
            for (int m = 0; m < TM; m++) {
                const int row_idx = tid / row_per_warp;
                const int row_in_tile = row_idx * TM + m;
                if (row_in_tile < BM) {
                    const float a_reg = As[row_in_tile][kk];
                    for (int n = 0; n < TN; n++) {
                        const int col_idx = tid % row_per_warp;
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < WARMUP; ++i) {
        gemm_128x64_kernel<tile_config::BM, tile_config::BN, tile_config::BK,
