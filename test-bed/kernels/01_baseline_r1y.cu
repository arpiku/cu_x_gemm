...
constexpr const char* VARIANT_DESC = "baseline_64x64_r1y";

template <int BM, int BN, int BK, int TM, int TN, int THREADS>
__global__ void gemm_baseline_kernel(
...
int main(int argc, char** argv) {
    ...
    const int THREADS = tile_config::THREADS;
    dim3 grid(
    ...
    for (int i = 0; i < WARMUP; ++i) {
        gemm_baseline_kernel<tile_config::BM, tile_config::BN, tile_config::BK,
