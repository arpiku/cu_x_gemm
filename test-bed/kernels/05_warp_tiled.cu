...
constexpr const char* VARIANT_DESC = "warp_tiled_16x16";

template <int BM, int BN, int BK, int WTM, int WTN, int THREADS>
    ...
    int* bank_conflict_count)
{
    ...
    const int warp_row_base = (warp_id % 2) * WTM;
    const int warp_col_base = (warp_id / 2) * WTN;
    
    __shared__ float As[BM][BK];
    __shared__ float Bs[BK][BN];
    
    float acc[WTM * WTN / 32] = {0.0f};
    
    for (int bk = 0; bk < K; bk += BK) {
        const int k_end = min(bk + BK, K);
        
        for (int load_idx = threadIdx.x; load_idx < BM * BK; load_idx += THREADS) {
        ...
        for (int load_idx = threadIdx.x; load_idx < BK * BN; load_idx += THREADS) {
            ...
            if (kk < k_end && col < N) {
                ...
                int bank_id = (load_row * BN + load_col) % 32;
                atomicAdd(&bank_access_count[bank_id], 1);
            }
        }
        ...
        for (int kk = 0; kk < BK; kk++) {
            for (int wm = 0; wm < WTM; wm++) {
                const int row_idx = warp_row_base + wm;
                const float a_reg = As[row_idx][kk];
                for (int wn = 0; wn < WTN; wn++) {
                    const int col_idx = warp_col_base + wn;
                    if (lane_id < (WTM * WTN / 32)) {
                        acc[wm * (WTN / 32) + wn] += a_reg * Bs[kk][col_idx];
                    }
                }
            }
        }
    ...
    for (int wm = 0; wm < WTM; wm++) {
        const int out_row = tile_row + warp_row_base + wm;
        for (int wn = 0; wn < WTN; wn++) {
            const int out_col = tile_col + warp_col_base + wn;
            if (out_row < M && out_col < N && lane_id < (WTM * WTN / 32)) {
                C[out_row * N + out_col] = alpha * acc[wm * (WTN / 32) + wn];
            }
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < WARMUP; ++i) {
        gemm_warp_tiled_kernel<tile_config::BM, tile_config::BN, tile_config::BK,
                              tile_config::WARP_TILE_M, tile_config::WARP_TILE_N, THREADS>
