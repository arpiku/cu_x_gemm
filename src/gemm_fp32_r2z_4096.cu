...
// r2z_4096: double-buffered SMEM + async B loads via cp.async
//
...
constexpr const char* const VARIANT_ID   = "r2z_4096";
...
void gemm_fp32_r2z_4096_kernel(
...
void launch_gemm_fp32_r2z_4096(
    const float* d_A,
    const float* d_B,
    float*       d_C,
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    using namespace r2z_4096_config;
    dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
    dim3 block(NUM_THREADS);

    gemm_fp32_r2z_4096_kernel<BM, BN, BK, WM, WN, WNITER, TM, TN, NUM_THREADS>
        <<<grid, block, 0, stream>>>(d_A, d_B, d_C, M, N, K, alpha, beta);
}

const char* get_variant_id_fp32_r2z_4096()   { return VARIANT_ID; }
const char* get_variant_desc_fp32_r2z_4096() { return VARIANT_DESC; }
