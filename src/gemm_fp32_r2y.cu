...
constexpr const char* const VARIANT_ID = "r2y_warp_tiling";
constexpr const char* const VARIANT_DESC = "128x128x16_warp4_128t";

template <int BM, int BN, int BK, int WM, int WN, int WNITER, int TM, int TN, int NUM_THREADS>
__global__ void __launch_bounds__(NUM_THREADS) gemm_fp32_r2y_kernel(
        ...
        for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
            ...
            for (uint resIdxM = 0; resIdxM < TM; ++resIdxM) {
                for (uint resIdxN = 0; resIdxN < TN; resIdxN += 4) {
                    float4 tmp = reinterpret_cast<float4*>(&C_interim[(threadRowInWarp * TM + resIdxM) * N +
    ...
    cudaStream_t stream)
{
    ...
    gemm_fp32_r2y_kernel<tile_config::BM, tile_config::BN, tile_config::BK,
...
const char* get_variant_id_fp32_r2y() { return VARIANT_ID; }
const char* get_variant_desc_fp32_r2y() { return VARIANT_DESC; }