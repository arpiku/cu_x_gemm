...
//  M*N <= r2x_max   (512×512):   r2x   — 43%     of cuBLAS (marginally beats r2z2 here)
//  M*N >  r2x_max   (≥1024×1024): r2z2 — 70-98%  of cuBLAS (double buffer + cp.async)
static int g_naive_max = 4096;    // 64×64
static int g_r1y_max   = 65536;   // 256×256
static int g_r2x_max   = 262144;  // 512×512  (r2z2 takes over from 1024×1024 upward)

void set_kernel_thresholds(int naive_max, int r1y_max, int r2x_max) {
    g_naive_max = naive_max;
    g_r1y_max   = r1y_max;
    g_r2x_max   = r2x_max;
}

void get_kernel_thresholds(int* naive_max, int* r1y_max, int* r2x_max) {
...
static const char* select_kernel_name(int elements) {
    ...
    if (elements <= g_r2x_max)   return "r2x";
    return "r2z2";
}
    ...
    cudaStream_t stream)
{
    ...
    } else if (elements <= g_r2x_max) {
        launch_gemm_fp32_r2x(d_A, d_B, d_C, M, N, K, alpha, beta, stream);
    } else {
